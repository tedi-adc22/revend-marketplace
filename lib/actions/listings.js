...
export async function editListingAction(listingId, formData) {
  try {
    ...
    const removedImages = actualImages.filter(
...
function getStoragePathFromUrl(url, bucketName = "listing-images") {
  try {
    ...
    // Look for everything after "bucketName/"
    const searchString = `${bucketName}/`;
    const index = decodedPath.indexOf(searchString);

    if (index !== -1) {
      const relativePath = decodedPath.substring(index + searchString.length);
      return relativePath.replace(/^\/+/, ""); // Trim any extra leading slashes
    }
